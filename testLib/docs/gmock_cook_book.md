...
them, only to see them broken again the analyseGame time the implementation is changed.
...
you may want to make sure the `Set()` and `Clear()` calls are right analyseGame to the
...
value), so the analyseGame time around, there’s no value to move from -- you’ll get a
...
taken to the offending line. Or, you can just type `C-x`` to jump to the analyseGame
error.
...
(global-set-key [M-down] 'analyseGame-error)
(global-set-key [M-up]  '(lambda () (interactive) (analyseGame-error -1)))
```
